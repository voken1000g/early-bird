<template>
  <layout-for-mobi class='set-voken-address mt-6 md:mt-8 lg:mt-12 xl:mt-14 2xl:mt-20'>
    <div class='py-8 px-6 bg-gradient-to-br from-red-100 to-orange-200 rounded-lg text-center'>
      ...
      <div v-if='vokenBalance.gt(0)'>
        ...
        <div v-show='showButtonSet' class='mt-8 flex justify-between space-x-2'>
          <button class='btn-swap' @click='saveVokenAddress'>
            {{ $t('referral.Set_VOKEN_address') }}
          </button>
        </div>

        <div v-show='showButtonNew' class='mt-4 flex justify-between space-x-2'>
          <button class='btn-generate' @click='newVokenAddress'>
    ...
    <div v-show='mnemonic' class='mt-6 py-6 px-4 bg-gradient-to-br from-blue-50 to-indigo-200 rounded-md'>
      ...
      <div class='mt-6 font-mono text-sm text-center'>
        <a target='_blank' :href="$t('link.Visit_Toolkit__VOKEN_wallet_generator.href')" class='a-info'>
          <fa :icon="['fas', 'wallet']" />
          {{ $t('link.Visit_Toolkit__VOKEN_wallet_generator.text') }}
        </a>
      </div>
    </div>
  </layout-for-mobi>
...
export default {
  ...
  watch: {
    async vokenAddress() {
      this.vInt = vokenAddress.toBN(this.vokenAddress).toString()
