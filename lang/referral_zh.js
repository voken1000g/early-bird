export default {
  Ready_to_dive_in_: '准备好深入了吗？',
  Join_the_referral_program_today: '今天就加入推荐计划。',
  Visit_Referral_System: '访问转介系统',

  Referral_system: '转介系统',
  Tell_a_friend_about__: '向朋友介绍 VOKEN，推荐用户可获得 10% + 12% 奖励（以 ETH 支付）',
  Share_your_referral_address_or_link__: '分享你的推荐地址或链接，你的朋友使用它将免费获得 1% - 10% 的随机奖励',
  You_can_receive_the_rewards_for__: '你可以从他和他的朋友那里获得每次付款的奖励',
  Payout_are_finalized_and_issued__: '在以太坊区块链上用 ETH 进行最终支付并立即发放',

  Set_your_VOKEN_address: '设置你的 VOKEN 地址',
  VOKEN_wallet_address: 'VOKEN 钱包地址',
  Not_a_VOKEN_wallet_address: '不是VOKEN钱包地址',
  The_address_is_already_exist: '该地址已经存在',
  Set_VOKEN_address: '设置 VOKEN 地址',
  You_dont_have_any_VOKEN: '你没有任何 VOKEN',
  Go_to_Early_bird_Sale: '进入早鸟销售',

  Referral_address_or_link: '推荐地址/链接',
  You_would_get_rewarded_from__: '你会从与你绑定的每一笔交易中得到回报。',
  Referral_address: '推荐地址',
  Referral_link: '推荐链接',
  Referral_rewards: '推荐奖励',
  Depth: '深度',
  Reward: '奖励',
  Transaction_hash: '交易散列',
  View: '查看',

  No_Referrals: '没有推荐'
}
